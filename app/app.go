...
func Run() {
	...
	api.Use(m.ApiVersioning())
	{
		...
		// repo initialization
		accessRepo := repo.NewAccessRepo(pgsqlDB)

		// initilizing usecases
		accessUseCases := usecases.NewRoleUseCases(accessRepo)

		// initalizin controllers
		accessControllers := controllers.NewAccessController(api, accessUseCases)

		// init the routes
		accessControllers.InitRoutes()
	}
